...
INFO_TABLE_STEMS = {
    "iso_link_sr_chairlift_202602161024",
    "iso_link_sr_log_type_202602161025",
    "iso_location_202602161022",
    "iso_observation_202602161023",
    "iso_product_status_202602161023",
    "msg_log_type_202602161025",
}
DEFAULT_STATUS_MAP = {
...
def load_info_datasheets(folder: str) -> dict[str, pd.DataFrame]:
    ...
    file_paths = discover_info_datasheet_files(base_dir)
...
def discover_info_datasheet_files(base_dir: Path) -> dict[str, Path]:
    candidates = [
        base_dir / "Info datasheets",
        base_dir.parent,
        base_dir,
    ]
    ...
    for folder in candidates:
        ...
        for file_path in files:
            if file_path.name.startswith("~$"):
                continue
            stem = file_path.stem
            if stem not in INFO_TABLE_STEMS:
                continue
            mtime = file_path.stat().st_mtime
            prev = selected.get(stem)
            if prev is None or mtime > prev[1]:
                selected[stem] = (file_path, mtime)

    return {name: path for name, (path, _) in selected.items()}
...
def main() -> None:
    ...
    info_file_map = discover_info_datasheet_files(script_dir)
            ...
            info_rows = [
                {
                    "bestand": name,
                    "rows": len(tbl),
                    "kolommen": len(tbl.columns),
                    "bron_pad": str(info_file_map[name]) if name in info_file_map else "",
                    "gewijzigd": pd.to_datetime(info_file_map[name].stat().st_mtime, unit="s")
                    if name in info_file_map
                    else pd.NaT,
                }
